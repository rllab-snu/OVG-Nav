...
class FMMPlanner:
    def __init__(self, args, traversible, num_rots, scale):
        self.args = args
        self.scale = scale
    ...
    def get_next_action(self, state, stg, agent_orientation):
        ...
        if relative_angle > self.act_rot:
            action = 3
        elif relative_angle < -self.act_rot:
            action = 2
        else:
            action = 1
        return action
    ...
    def _find_nearest_goal(self, goal):
        ...
        rots = 360 // self.act_rot
        planner4 = FMMPlanner(self.args, traversible4, rots, 1)
