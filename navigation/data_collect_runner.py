...
class Runner:
    ...
    def get_data(self, env_idx, tot_env_num):
        ...
        self.goal_category_feat = self.common_sense_model.clip.get_text_feat(gibson_goal_obj_names).type(
            torch.float32).cpu()
        ...
        if self.data_type == 'train':
            interval = 5
            valid_traj_list = random.sample(valid_traj_list, len(valid_traj_list)//interval)
        ...
        for traj in valid_traj_list[0:len(valid_traj_list)]:
            ...
            if shortest_path_length < 3.0:
                easy_success.append(success)
                easy_spl.append(spl)
            elif shortest_path_length < 5.0:
