...
import numpy as np
from utils.obj_category_info import d3_40_colors_rgb
...
class SemanticPredRedNet:
    ...
    def __init__(self, args):
        self.args = args
        # Setup model
        self.model = RedNet.from_config()
        self.model.eval()

        ckpt = torch.load(self.args.segmentation_model)
        model_state = {
            k.replace("module.", ""): v for k, v in ckpt["model_state"].items()
        }
        self.model.load_state_dict(model_state)
        # Convert model to device
        sem_gpu_id = self.args.model_gpu
        if sem_gpu_id == -1:
        ...
        # Normalization
        self.input_max_depth = 10. #self.args.max_depth
        self.input_min_depth = 0.5 #self.args.min_depth
        ...
        self.n_classes = 21
        self.sem_pred_prob_thr = 0.9
        self.depth_thresh = [1.0, 5.0]
    ...
    def process_predictions(self, predictions, raw_depth):
        ...
        semantic_inputs = torch.zeros(
            B, self.n_classes + 1, H, W, device=predictions.device
        )
        is_confident = torch.any(
            predictions >= self.sem_pred_prob_thr, dim=1
        )  # (B, H, W)
        ...
        is_within_thresh = (raw_depth[:, 0] >= self.depth_thresh[0]) & (
            raw_depth[:, 0] <= self.depth_thresh[1]
        )
        ...
        semantic_inputs[:, self.n_classes] = predictions_argmax == THDA_BACKGROUND
        return semantic_inputs


    def visualize_rednet_pred(self, pred):
        if torch.is_tensor(pred):
            pred = np.transpose(np.array(pred.cpu()), axes=(1, 2, 0))

        k_prime = np.argmax(pred, axis=2)
        vis_pred = d3_40_colors_rgb[k_prime]
        vis_pred[k_prime == 21] = 0
